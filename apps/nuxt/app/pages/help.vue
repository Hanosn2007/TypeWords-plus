...
const questions = [
  { q: '数据保存在哪里？怎么在不同设备继续学习？', a: ['学习进度先保存在当前浏览器。登录本站账号后，可在“同步”页上传或取回云端进度；本站账号与原 TypeWords 服务不互通。', '自动同步在窗口失焦或页面隐藏时触发，也可以点击“立即同步”。换设备前请确认同步成功；只保存在本机的进度不会自动出现在另一台设备。', '两端同时修改产生版本冲突时，会显示比较界面，由你确认如何处理，不按设备时间自动覆盖。'] },
  { q: '进度看起来变少或不见了，怎么办？', a: ['先确认网址、浏览器、浏览器个人资料和登录账号是否与上次一致。先不要清除网站数据、重置词书或用较旧记录覆盖。', '检查“同步”状态和可用的历史记录；也可以先在设置中导出当前数据，再反馈问题。浏览器本地保存与云端同步是两件事，看到旧云端进度不一定代表本机数据已经消失。', '无痕窗口、清除网站数据、磁盘或浏览器存储空间不足都可能影响本地保存。出现保存失败提示时，先保留页面并导出数据。'] },
