...
- “账号与数据”统一登录、离线 ZIP 导入导出和恢复；北京时间每日仅留最新历史，云端3个有效日期，本机1–30个（默认3）。
- 保留文章练习及导入入口。

## 已知边界

维护目标为 Web 版本，VS Code 客户端、原上游账号及 Supabase 后台同步已退役。旧 Supabase 配置只用于用户主动的只读迁移；历史文件仍可识别。部分词典、语音和辅助资源仍依赖外部服务；尚未完成资源自托管或完整离线能力。
